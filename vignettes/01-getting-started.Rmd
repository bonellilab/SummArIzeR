...
This is a basic example which shows you how to run `SummArIzeR`. We
...
See: [Detailed workflow](../articles/02-detailed_workflow.html)
```{r, view, include=T}
...
```{r, cluster_network, eval=F, fig.width=15, fig.height=12}
plot<-TRUplotIgraph(Termlist_all, ts  = 0.3)
#htmltools::tagList(plot)
```
```{r, cluster_network_code, include=F, fig.width=15, fig.height=12}
plot<-TRUplotIgraph(Termlist_all, ts  = 0.3)
#htmltools::tagList(plot)
```
